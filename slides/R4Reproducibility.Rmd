...
- Knit to different output types

- Change local and global chunk options
...
- Read in excel data and connect to a database
...
`tidyverse` provides a coherent ecosystem for these tasks via the `dplyr` package!  

- Good function defaults

- All work on `tibbles` (special data frames)
...
- Use `select()` and `rename()` to modify columns of a `tibble`
...
- `unite()` & `separate()` nice for dealing with columns  
...
- Categorical variable - entries are a label or attribute   
    ...
    + Done with contingency tables via `table()`
...
- An object with three dimension! `[ , , ]`
...
- Return the survived table for males that embarked at "Q"
...
- Numerical summaries
    ...
    + Subset to obtain conditional tables   
...
<div style="float: left; width: 40%;">
```{r}
var(titanicData$fare, na.rm = TRUE)
sd(titanicData$fare, na.rm = TRUE)
```
</div>
<div style="float: right; width: 55%;">
```{r}
IQR(titanicData$fare, na.rm = TRUE)
quantile(titanicData$fare, probs = c(0.1, 0.2), na.rm = TRUE)
```
</div>
<!--Comment-->
...
```{r}
cov(titanicData$fare, titanicData$age, use = "complete.obs")
cor(titanicData$fare, titanicData$age, use = "complete.obs")
```
...
```{r}
titanicData %>% 
  tidyr::drop_na(survived, embarked) %>%
...
- Across subgroups with `dplyr::group_by()`, `dplyr::summarize()`, and `dplyr::across()`
...
```{r}
titanicData <- titanicData %>% mutate(mySurvived = factor(mySurvived, levels = c("Survived", "Died")))
table(titanicData$mySurvived)
```
...
```{r,eval = TRUE, echo = TRUE, out.width = "650px", fig.align='center', out.height = '350px'}
g <- ggplot(sumData) 
g + geom_bar(stat = "identity", aes(x = survived, y = count, fill = sex))
```
...
# `ggplot2` Smoothed Histograms, Boxplots, & Scatter Plots  
Justin Post
...
To create plots use the same process:
...
- Create and customize density plots, boxplots, and scatter plots!
...
# `ggplot2` Faceting, Themes, and More  
Justin Post
